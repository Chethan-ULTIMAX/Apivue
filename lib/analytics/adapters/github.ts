...
export function analyzeGitHub(profile: NormalizedProfile<NormalizedGitHubData>, fetchedAt?: string): AnalyticsResult {
  ...
  const stars = repositories.map(repo => repo.stargazers_count ?? 0);
  const forks = repositories.map(repo => repo.forks_count ?? 0);
  const metrics: AnalyticsMetric[] = [
    ...
    { id: "highest-stars", label: "Highest repository stars", value: maximum(stars) ?? "Unavailable", category: "repositories", calculation: "calculated", comparable: true, availability: repositories.length ? "available" : "insufficient-data" },
  ];

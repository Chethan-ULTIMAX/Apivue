...
const KEY = "apivue:v1";
const emptyStore = (): WorkspaceStore => ({ workspaces: [], profiles: [], comparisons: [] });
...
export function loadStore(): WorkspaceStore {
  ...
  try {
    ...
    return parsed && Array.isArray(parsed.workspaces) ? parsed : emptyStore();
...
export function saveProfile(result: AnalyticsResult, snapshot: AnalyticsSnapshot): WorkspaceStore {
  ...
  store = { ...store, profiles: [profile, ...store.profiles.filter(p => !(p.platformId === profile.platformId && p.subjectId === profile.subjectId))] };
...
export function saveComparison(platformId: string, left: string, right: string): WorkspaceStore {
  ...
  store = { ...store, comparisons: [comparison, ...store.comparisons] };
  const workspace = store.workspaces.find(w => w.id === store.activeWorkspaceId) ?? store.workspaces[0];
...
export function createWorkspace(name: string, ownerId = "local-user"): WorkspaceStore {
  const store = ensureDefaultWorkspace(loadStore(), ownerId); const now = new Date().toISOString();
  const workspace: Workspace = { id: `ws_${crypto.randomUUID()}`, name: name.trim() || "Untitled Workspace", ownerId, members: [{ userId: ownerId, role: "owner" }], savedProfileIds: [], comparisonIds: [], createdAt: now, updatedAt: now };
  const next = { ...store, workspaces: [...store.workspaces, workspace], activeWorkspaceId: workspace.id }; saveStore(next); return next;
}
